// Generated by gltf-typescript-generator from
// github.com/KhronosGroup/glTF/blob/2a29b03d84a2453388d03309ee77033977c01d4b/specification/2.0/schema/glTF.schema.json
// Hand-edited by Uri Shaked
...
/**
 * Array of size `accessor.sparse.count` times number of components storing the displaced accessor attributes
 * pointed by `accessor.sparse.indices`.
 */
export interface AccessorSparseValues {
  /**
   * The index of the bufferView with sparse values. Referenced bufferView can't have ARRAY_BUFFER or
 ...
 * An accessor provides a typed view into a bufferView or a subset of a bufferView similar to how WebGL's
...
export interface Animation {
  ...
  /**
   * An array of samplers that combines input and output accessors with an interpolation algorithm to
...
export interface MeshPrimitive {
  /**
   * A dictionary object, where each key corresponds to mesh attribute semantic and each value is the index
  ...
  mode?: 0 | 1 | 2 | 3 | 4 | 5 | 6;
  /**
   * An array of Morph Targets, each  Morph Target is a dictionary mapping attributes (only `POSITION`, `NORMAL`,
   * and `TANGENT` supported) to their deviations in the Morph Target.
   */
  targets?: Array<{
    [k: string]: GlTfId;
  }>;
 ...
 * (TRS) properties. TRS properties are converted to matrices and postmultiplied in the `T * R * S` order to compose
